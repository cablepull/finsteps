...
const calculateFullBoundingBox = (svg, padding = 40) => {
    ...
    if (shouldUseElementCalculation) {
        ...
        if (topLevelElements.length === 0) {
            ...
            topLevelElements.push(...Array.from(svg.querySelectorAll("g, rect, circle, ellipse, line, polyline, polygon, path, text")));
        }
...
export const createBasicCameraHandle = (diagram) => {
        ...
        fit(target, options) {
            ...
            try {
                ...
                if (bbox.height < minDimension) {
                    bbox = new DOMRect(bbox.x, bbox.y - minDimension / 2, bbox.width || minDimension, minDimension);
                }
                if (bbox.width < minDimension) {
                    bbox = new DOMRect(bbox.x - minDimension / 2, bbox.y, minDimension, bbox.height || minDimension);
                }
                ...
                for (const adjacentNode of adjacentNodes) {
                    try {
                        ...
                        unionBbox = new DOMRect(minX, minY, maxX - minX, maxY - minY);
                    }
                    catch {
