...
export interface DiagramAdapter {
    render(options: {
        mountEl: HTMLElement;
        mermaidText: string;
    }): Promise<DiagramHandle> | DiagramHandle;
}
...
export type ActionHandler = (context: ActionContext, action: ActionDefinition) => void | Promise<void>;
...
export interface Controller {
    ...
    on(event: "stepchange" | "actionerror" | "error" | "render" | "actionstart" | "actioncomplete" | "astchange", handler: (payload: unknown) => void): () => void;
    getDeps?(): {
        diagram: DiagramHandle;
        camera?: CameraHandle;
        overlay?: OverlayHandle;
...
export interface ControllerHooks {
    ...
    onActionComplete?: (action: ActionDefinition, result: {
        success: boolean;
        error?: Error;
    }) => void;
