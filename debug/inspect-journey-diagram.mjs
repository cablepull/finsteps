// Playwright script to inspect Journey diagram and see actual SVG structure
...
try {
  ...
  console.log('\n=== Inspecting Journey Diagram Structure ===\n');
  ...
  const inspection = await page.evaluate(() => {
    ...
    // Extract step names from the example
    const stepNames = ['Landing Page', 'Pricing Page', 'Signup Form', 'Email Verification', 'Welcome Tour', 'Dashboard'];
    const stepMatches = {};
    for (const stepName of stepNames) {
      // Try different ways to find the step
      const exactId = svg.querySelector(`#${stepName}`);
      const containsId = Array.from(svg.querySelectorAll(`[id*="${stepName}"]`));
      
      // Find elements whose text content contains the step name
      const allElements = Array.from(svg.querySelectorAll('*'));
      const textContains = allElements.filter(el => {
        const text = el.textContent || '';
        return text.toLowerCase().includes(stepName.toLowerCase());
      });
  ...
  console.log('\n=== Step Matches ===');
  for (const [stepName, matches] of Object.entries(inspection.stepMatches)) {
    console.log(`\n${stepName}:`);
    if (matches.exactId) {
    ...
    if (matches.textContains.length > 0) {
      ...
      console.log(`  ❌ No text content containing "${stepName}"`);
    }
  ...
  fs.writeFileSync('debug/journey-inspection.json', JSON.stringify(inspection, null, 2));
  console.log('\n✅ Full inspection saved to debug/journey-inspection.json');
