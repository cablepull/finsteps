// Playwright script to test C4Context diagram fallback logic
...
try {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('[MermaidDiagram]') || text.includes('[C4Strategy]') || text.includes('[TargetResolver]')) {
      logs.push(text);
      console.log('Console:', text);
    }
  });
  
  console.log('\n=== Testing C4Context Diagram ===\n');
  ...
  const state = await page.evaluate(() => {
    ...
    // Check for specific aliases (based on c4context example)
    ...
    for (const alias of aliases) {
      ...
      if (svg.getAttribute('data-id') === alias) {
        aliasResults[alias] = {
          ...
          tagName: svg.tagName,
          id: svg.id || null,
          className: svg instanceof SVGElement ? 
    ...
    return {
      ...
      rootDataId: svg.getAttribute('data-id'),
      aliasResults,
    };
  });
  ...
  console.log('\n=== Testing Navigation ===');
  const buttons = ['overview', 'user', 'webapp', 'auth'];
  ...
  fs.writeFileSync('debug/c4context-test-state.json', JSON.stringify({ state, logs }, null, 2));
  console.log('\n✅ Full test state saved to debug/c4context-test-state.json');
