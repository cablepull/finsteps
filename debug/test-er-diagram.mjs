// Playwright script to test ER diagram
...
try {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('[MermaidDiagram]') || text.includes('[ERDiagramStrategy]') || text.includes('[TargetResolver]')) {
      logs.push(text);
  ...
  const state = await page.evaluate(() => {
    ...
    // Get all entities from the example (check what's in the HTML - they're uppercase in Mermaid)
    const aliases = ['CUSTOMER', 'ORDER', 'PRODUCT', 'ORDER_ITEM'];
    const aliasResults = {};
  ...
  // Test clicking buttons (these use lowercase in the HTML)
  console.log('\n=== Testing Navigation ===');
  const buttons = ['overview', 'customer', 'order', 'product', 'orderitem'];
  ...
  fs.writeFileSync('debug/er-test-state.json', JSON.stringify({ state, logs }, null, 2));
  console.log('\n✅ Full test state saved to debug/er-test-state.json');
