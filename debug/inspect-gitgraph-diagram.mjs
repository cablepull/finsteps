// Playwright script to inspect Git Graph diagram and see actual SVG structure
...
try {
  console.log('Navigating to Git Graph example...');
  const response = await page.goto('http://localhost:5173/examples/gitgraph/', { waitUntil: 'networkidle', timeout: 30000 });
  ...
  console.log('\n=== Inspecting Git Graph Diagram Structure ===\n');
  ...
  const inspection = await page.evaluate(() => {
    ...
    // Extract branch/commit names from the example
    const branchNames = ['main', 'develop', 'feature', 'hotfix'];
    const branchMatches = {};
    for (const branchName of branchNames) {
      // Try different ways to find the branch
      const exactId = svg.querySelector(`#${branchName}`);
      const containsId = Array.from(svg.querySelectorAll(`[id*="${branchName}"]`));
      
      // Find elements whose text content contains the branch name
      const allElements = Array.from(svg.querySelectorAll('*'));
      const textContains = allElements.filter(el => {
        const text = el.textContent || '';
        return text.toLowerCase().includes(branchName.toLowerCase());
      });
  ...
  console.log('\n=== Branch Matches ===');
  for (const [branchName, matches] of Object.entries(inspection.branchMatches)) {
    console.log(`\n${branchName}:`);
    if (matches.exactId) {
    ...
    if (matches.textContains.length > 0) {
      ...
      console.log(`  ❌ No text content containing "${branchName}"`);
    }
  ...
  fs.writeFileSync('debug/gitgraph-inspection.json', JSON.stringify(inspection, null, 2));
  console.log('\n✅ Full inspection saved to debug/gitgraph-inspection.json');
