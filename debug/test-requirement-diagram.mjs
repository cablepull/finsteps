// Playwright script to test Requirement diagram
...
try {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('[MermaidDiagram]') || text.includes('[RequirementStrategy]') || text.includes('[TargetResolver]')) {
      logs.push(text);
      console.log('Console:', text);
    }
  });
  
  console.log('\n=== Testing Requirement Diagram ===\n');
  ...
  const state = await page.evaluate(() => {
    ...
    // Get requirement/element names from the example
    const requirementNames = ['req1', 'req2', 'func1'];
    const requirementResults = {};
    for (const reqName of requirementNames) {
      const elements = Array.from(svg.querySelectorAll(`[data-id="${reqName}"]`));
      if (svg.getAttribute('data-id') === reqName) {
  ...
  console.log('\n=== Requirement Results ===');
  for (const [reqName, result] of Object.entries(state.requirementResults)) {
    if (result.found) {
      console.log(`✅ ${reqName}: Found`);
      if (result.isRoot) {
  ...
  console.log('\n=== Testing Navigation ===');
  const buttons = ['overview', 'req1', 'req2', 'func1'];
  ...
  fs.writeFileSync('debug/requirement-test-state.json', JSON.stringify({ state, logs }, null, 2));
  console.log('\n✅ Full test state saved to debug/requirement-test-state.json');
