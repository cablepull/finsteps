// Playwright script to test Pie Chart diagram
...
try {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('[MermaidDiagram]') || text.includes('[PieChartStrategy]') || text.includes('[TargetResolver]')) {
      logs.push(text);
      console.log('Console:', text);
    }
  });
  
  console.log('\n=== Testing Pie Chart Diagram ===\n');
  ...
  const state = await page.evaluate(() => {
    ...
    // Get slice names from the example (check what's in the HTML)
    // The example uses: Desktop, Mobile, Tablet, Other
    const sliceNames = ['Desktop', 'Mobile', 'Tablet', 'Other'];
    const sliceResults = {};
    for (const sliceName of sliceNames) {
      const elements = Array.from(svg.querySelectorAll(`[data-id="${sliceName}"]`));
      if (svg.getAttribute('data-id') === sliceName) {
        sliceResults[sliceName] = {
          found: true,
          tagName: svg.tagName,
          id: svg.id || null,
          isRoot: true
  ...
  console.log('\n=== Slice Results ===');
  for (const [sliceName, result] of Object.entries(state.sliceResults)) {
    if (result.found) {
      console.log(`✅ ${sliceName}: Found`);
      if (result.isRoot) {
      ...
      console.log(`❌ ${sliceName}: NOT FOUND`);
    }
  }
  
  // Test clicking buttons
  console.log('\n=== Testing Navigation ===');
  const buttons = ['overview', 'desktop', 'mobile', 'tablet', 'other'];
  ...
  fs.writeFileSync('debug/pie-test-state.json', JSON.stringify({ state, logs }, null, 2));
  console.log('\n✅ Full test state saved to debug/pie-test-state.json');
