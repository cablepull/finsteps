// Playwright script to inspect Pie Chart diagram and see actual SVG structure
...
try {
  console.log('Navigating to Pie Chart example...');
  const response = await page.goto('http://localhost:5173/examples/pie/', { waitUntil: 'networkidle', timeout: 30000 });
  ...
  console.log('\n=== Inspecting Pie Chart Diagram Structure ===\n');
  ...
  const inspection = await page.evaluate(() => {
    ...
    // Extract slice names from the example
    const sliceNames = ['Desktop', 'Mobile', 'Tablet', 'Other'];
    const sliceMatches = {};
    for (const sliceName of sliceNames) {
      // Try different ways to find the slice
      const exactId = svg.querySelector(`#${sliceName}`);
      const containsId = Array.from(svg.querySelectorAll(`[id*="${sliceName}"]`));
      
      // Find elements whose text content contains the slice name
      const allElements = Array.from(svg.querySelectorAll('*'));
      const textContains = allElements.filter(el => {
        const text = el.textContent || '';
        return text.toLowerCase().includes(sliceName.toLowerCase());
      });
  ...
  console.log('\n=== Slice Matches ===');
  for (const [sliceName, matches] of Object.entries(inspection.sliceMatches)) {
    console.log(`\n${sliceName}:`);
    if (matches.exactId) {
    ...
    if (matches.textContains.length > 0) {
      ...
      console.log(`  ❌ No text content containing "${sliceName}"`);
    }
  ...
  fs.writeFileSync('debug/pie-inspection.json', JSON.stringify(inspection, null, 2));
  console.log('\n✅ Full inspection saved to debug/pie-inspection.json');
