// Playwright script to inspect ER diagram and see actual SVG structure
...
try {
  console.log('Navigating to ER example...');
  const response = await page.goto('http://localhost:5173/examples/er/', { waitUntil: 'networkidle', timeout: 30000 });
  ...
  console.log('\n=== Inspecting ER Diagram Structure ===\n');
  ...
  const inspection = await page.evaluate(() => {
    ...
    // Extract entity names from the example
    const entityNames = ['Customer', 'Order', 'Product', 'OrderItem'];
    const entityMatches = {};
    for (const entityName of entityNames) {
      // Try different ways to find the entity
      const exactId = svg.querySelector(`#${entityName}`);
      const containsId = Array.from(svg.querySelectorAll(`[id*="${entityName}"]`));
      
      // Find elements whose text content contains the entity name
  ...
  console.log('\n=== Entity Matches ===');
  for (const [entityName, matches] of Object.entries(inspection.entityMatches)) {
    console.log(`\n${entityName}:`);
    if (matches.exactId) {
    ...
    if (matches.textContains.length > 0) {
      ...
      console.log(`  ❌ No text content containing "${entityName}"`);
    }
  ...
  fs.writeFileSync('debug/er-inspection.json', JSON.stringify(inspection, null, 2));
  console.log('\n✅ Full inspection saved to debug/er-inspection.json');
