// Playwright script to inspect C4Container diagram and see actual SVG structure
...
try {
  ...
  console.log('\n=== Inspecting C4Container Diagram Structure ===\n');
  ...
  const inspection = await page.evaluate(() => {
    ...
    const groupsWithClasses = allGroups.filter(g => {
      ...
      hasShapes: !!g.querySelector('rect, circle, ellipse, polygon, path'),
    ...
    // From the example: user, webapp, web, api, db
  ...
  inspection.groupsWithClasses.groups.forEach((group, idx) => {
    console.log(`${idx + 1}. ${group.tagName}.${group.className.split(' ')[0] || 'no-class'}${group.id ? `#${group.id}` : ''}${group.dataId ? `[data-id="${group.dataId}"]` : ''}${group.hasShapes ? ' (has shapes)' : ''}`);
  ...
  fs.writeFileSync('debug/c4container-inspection.json', JSON.stringify(inspection, null, 2));
  console.log('\n✅ Full inspection saved to debug/c4container-inspection.json');
