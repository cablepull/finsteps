// Playwright script to test Quadrant Chart diagram
...
try {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('[MermaidDiagram]') || text.includes('[QuadrantChartStrategy]') || text.includes('[TargetResolver]')) {
      logs.push(text);
      console.log('Console:', text);
    }
  });
  
  console.log('\n=== Testing Quadrant Chart Diagram ===\n');
  ...
  const state = await page.evaluate(() => {
    ...
    // Get quadrant item names from the example
    const itemNames = ['Leader', 'Challenger', 'Niche', 'Innovator'];
    const itemResults = {};
    for (const itemName of itemNames) {
      const elements = Array.from(svg.querySelectorAll(`[data-id="${itemName}"]`));
      if (svg.getAttribute('data-id') === itemName) {
  ...
  console.log('\n=== Item Results ===');
  for (const [itemName, result] of Object.entries(state.itemResults)) {
    if (result.found) {
  ...
  console.log('\n=== Testing Navigation ===');
  const buttons = ['overview', 'leader', 'challenger', 'niche', 'innovator'];
  ...
  fs.writeFileSync('debug/quadrant-test-state.json', JSON.stringify({ state, logs }, null, 2));
  console.log('\n✅ Full test state saved to debug/quadrant-test-state.json');
