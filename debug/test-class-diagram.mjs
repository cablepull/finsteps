// Playwright script to test Class diagram
...
try {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('[MermaidDiagram]') || text.includes('[ClassDiagramStrategy]') || text.includes('[TargetResolver]')) {
      logs.push(text);
  ...
  const state = await page.evaluate(() => {
    ...
    // Get all classes from the example (check what's in the HTML)
    const aliases = ['Vehicle', 'Car', 'Motorcycle', 'Wheel'];
    const aliasResults = {};
  ...
  console.log('\n=== Testing Navigation ===');
  const buttons = ['overview', 'vehicle', 'car', 'motorcycle', 'wheel'];
  ...
  fs.writeFileSync('debug/class-test-state.json', JSON.stringify({ state, logs }, null, 2));
  console.log('\n✅ Full test state saved to debug/class-test-state.json');
