// Playwright script to test Git Graph diagram
...
try {
  ...
  page.on('console', msg => {
    const text = msg.text();
    if (text.includes('[MermaidDiagram]') || text.includes('[GitGraphStrategy]') || text.includes('[TargetResolver]')) {
      logs.push(text);
      console.log('Console:', text);
    }
  });
  
  console.log('\n=== Testing Git Graph Diagram ===\n');
  ...
  const state = await page.evaluate(() => {
    ...
    // Get all commits/branches from the example (check what's in the HTML)
    // The example uses: main, feature, and merge (not develop/hotfix)
    const aliases = ['main', 'feature', 'merge', 'Initial'];
    const aliasResults = {};
    for (const alias of aliases) {
      const elements = Array.from(svg.querySelectorAll(`[data-id="${alias}"]`));
      // Also check if SVG root has the data-id
      if (svg.getAttribute('data-id') === alias) {
        aliasResults[alias] = {
          found: true,
          tagName: svg.tagName,
          id: svg.id || null,
  ...
  console.log('\n=== Testing Navigation ===');
  const buttons = ['overview', 'main', 'feature', 'merge'];
  ...
  fs.writeFileSync('debug/gitgraph-test-state.json', JSON.stringify({ state, logs }, null, 2));
  console.log('\n✅ Full test state saved to debug/gitgraph-test-state.json');
