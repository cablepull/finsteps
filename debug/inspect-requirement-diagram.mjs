// Playwright script to inspect Requirement diagram and see actual SVG structure
...
try {
  console.log('Navigating to Requirement diagram example...');
  const response = await page.goto('http://localhost:5173/examples/requirement/', { waitUntil: 'networkidle', timeout: 30000 });
  ...
  console.log('\n=== Inspecting Requirement Diagram Structure ===\n');
  ...
  const inspection = await page.evaluate(() => {
    ...
    // Extract requirement/element names from the example
    const itemNames = ['req1', 'req2', 'func1'];
    const itemMatches = {};
    ...
    // Get all groups with requirement/function classes
    const allGroups = Array.from(svg.querySelectorAll('g'));
    const requirementGroups = allGroups.filter(g => {
      const cn = g.className?.baseVal || g.className || '';
      return typeof cn === 'string' && (cn.includes('requirement') || cn.includes('function'));
    });
  ...
  console.log(`\n=== Requirement/Function Groups (${inspection.requirementGroups.total}) ===`);
  inspection.requirementGroups.groups.forEach((g, idx) => {
    console.log(`${idx + 1}. g${g.id ? `#${g.id}` : ''}${g.dataId ? `[data-id="${g.dataId}"]` : ''} (${g.className.split(' ')[0] || 'no-class'})`);
  ...
  fs.writeFileSync('debug/requirement-inspection.json', JSON.stringify(inspection, null, 2));
  console.log('\n✅ Full inspection saved to debug/requirement-inspection.json');
