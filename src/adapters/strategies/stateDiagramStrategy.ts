...
export class StateDiagramStrategy extends BaseDiagramStrategy {
  ...
  extractNodeIds(svg: SVGSVGElement): Map<string, SVGElement> {
    ...
    for (let groupIdx = 0; groupIdx < stateGroupsNodes.length; groupIdx++) {
      const group = stateGroupsNodes[groupIdx] as SVGElement;
  ...
  findAdjacentElements(target: SVGGraphicsElement, svg: SVGSVGElement): SVGGraphicsElement[] {
    ...
    if (targetClassName && (targetClassName.includes('state') || targetClassName.includes('stateNode'))) {
      ...
      const allTransitions = Array.from(svg.querySelectorAll<SVGGraphicsElement>('g[class*="transition"][data-id], g[class*="edge"][data-id], path[class*="edge"]'));
      const connectedStates: SVGGraphicsElement[] = [];
      ...
      try {
        ...
        for (const transition of allTransitions) {
          try {
            ...
            const intersects = 
              transitionBbox.x < targetBbox.x + targetBbox.width &&
            ...
            if (intersects) {
              ...
              const allStates = Array.from(svg.querySelectorAll<SVGGraphicsElement>('g[class*="state"][data-id], g[class*="stateNode"][data-id]'));
              for (const state of allStates) {
                if (state === target) continue;
                if (connectedStates.includes(state)) continue;
                
                try {
                  const stateBbox = state.getBBox();
                  const stateIntersects = 
                    transitionBbox.x < stateBbox.x + stateBbox.width &&
