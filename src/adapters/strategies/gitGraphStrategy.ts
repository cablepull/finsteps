...
export class GitGraphStrategy extends BaseDiagramStrategy {
  ...
  findAdjacentElements(target: SVGGraphicsElement, svg: SVGSVGElement): SVGGraphicsElement[] {
    ...
    if (targetClassName && targetClassName.includes('commit')) {
      ...
      const allBranches = Array.from(svg.querySelectorAll<SVGGraphicsElement>('g[class*="branch"][data-id], path[class*="branch"], path[class*="edge"]'));
      const connectedCommits: SVGGraphicsElement[] = [];
      ...
      try {
        ...
        for (const branch of allBranches) {
          try {
            ...
            const intersects = 
              branchBbox.x < targetBbox.x + targetBbox.width &&
            ...
            if (intersects) {
              ...
              const allCommits = Array.from(svg.querySelectorAll<SVGGraphicsElement>('g[class*="commit"][data-id], circle[class*="commit"][data-id]'));
              for (const commit of allCommits) {
                if (commit === target) continue;
                if (connectedCommits.includes(commit)) continue;
                
                try {
                  const commitBbox = commit.getBBox();
                  const commitIntersects = 
                    branchBbox.x < commitBbox.x + commitBbox.width &&
