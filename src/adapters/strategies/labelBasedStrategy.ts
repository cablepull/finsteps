...
const isNumericLabel = (label: string): boolean => /^-?\d+(\.\d+)?$/.test(label);

const normalizeDataId = (raw: string): string => {
...
export class LabelBasedStrategy extends BaseDiagramStrategy {
  constructor(
    ...
    private options: LabelBasedStrategyOptions = {}
  ) {
    super();
  }
  ...
  extractNodeIds(svg: SVGSVGElement): Map<string, SVGElement> {
    ...
    for (const textEl of texts) {
      ...
      if (!label) continue;
      if (this.options.skipNumericLabels && isNumericLabel(label)) continue;
  ...
  private findTargetGroupForText(textEl: SVGTextElement, svg: SVGSVGElement): SVGElement | null {
    ...
    while (current && current !== svg) {
      ...
      if (parent.tagName.toLowerCase() === "g") {
        const svgEl = parent as unknown as SVGElement;
