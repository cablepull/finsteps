  ...
  findAdjacentElements(target: SVGGraphicsElement, svg: SVGSVGElement): SVGGraphicsElement[] {
    ...
    if (targetClassName && (targetClassName.includes('point') || targetClassName.includes('data'))) {
      ...
      try {
        ...
        // Find all quadrants
        const allQuadrants = Array.from(svg.querySelectorAll<SVGGraphicsElement>('g[class*="quadrant"]'));
        
        for (const quadrant of allQuadrants) {
          try {
            const quadrantBbox = quadrant.getBBox();
            if (
              targetCenterX >= quadrantBbox.x &&
              ...
              const quadrantPoints = Array.from(quadrant.querySelectorAll<SVGGraphicsElement>('g[class*="point"][data-id], circle[class*="point"][data-id], g[class*="data"][data-id], circle[class*="data"][data-id]'));
        ...
        const allPoints = Array.from(svg.querySelectorAll<SVGGraphicsElement>('g[class*="point"][data-id], circle[class*="point"][data-id], g[class*="data"][data-id], circle[class*="data"][data-id]'));
    ...
    if (targetClassName && targetClassName.includes('quadrant')) {
      const quadrantPoints = Array.from(target.querySelectorAll<SVGGraphicsElement>('g[class*="point"][data-id], circle[class*="point"][data-id], g[class*="data"][data-id], circle[class*="data"][data-id]'));
