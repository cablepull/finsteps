...
export class ERDiagramStrategy extends BaseDiagramStrategy {
  ...
  findAdjacentElements(target: SVGGraphicsElement, svg: SVGSVGElement): SVGGraphicsElement[] {
    ...
    if (targetClassName && targetClassName.includes('entity')) {
      ...
      const allRelations = Array.from(svg.querySelectorAll<SVGGraphicsElement>('g[class*="relationship"][data-id], g[class*="edge"][data-id], path[class*="edge"]'));
      const connectedEntities: SVGGraphicsElement[] = [];
        ...
        for (const relation of allRelations) {
          try {
            ...
            if (intersects) {
              ...
              const allEntities = Array.from(svg.querySelectorAll<SVGGraphicsElement>('g[class*="entity"][data-id]'));
              for (const entity of allEntities) {
                if (entity === target) continue;
                if (connectedEntities.includes(entity)) continue;
                
                try {
                  const entityBbox = entity.getBBox();
                  const entityIntersects = 
                    relationBbox.x < entityBbox.x + entityBbox.width &&
