...
export class BlockDiagramStrategy extends BaseDiagramStrategy {
  ...
  findAdjacentElements(target: SVGGraphicsElement, svg: SVGSVGElement): SVGGraphicsElement[] {
    ...
    if (targetClassName && (targetClassName.includes('block') || targetClassName.includes('node'))) {
      // Find all edge paths
      const allEdges = Array.from(svg.querySelectorAll<SVGPathElement>('path.edge, path[class*="edge"], g.edge path, path[id*="edge"]'));
      const connectedBlocks: SVGGraphicsElement[] = [];
        ...
        for (const edgePath of allEdges) {
          try {
            ...
            if (edgeIntersectsTarget || distanceToCenter < threshold) {
              ...
              const allBlocks = Array.from(svg.querySelectorAll<SVGGraphicsElement>('g.block[data-id], g.node[data-id]'));
              
              for (const block of allBlocks) {
                if (block === target) continue;
                if (connectedBlocks.includes(block)) continue;
                ...
                try {
                  ...
                  const blockIntersectsEdge = 
                    edgeBbox.x < blockBbox.x + blockBbox.width &&
